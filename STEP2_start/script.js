...
URLList[11][0] = document.title
URLList[11][1] = url[url.length - 1]
...
const commentsList = [
	"[ランゲル]から通信あり",
	"こちらランゲル聞こえる？",
	"どうやら僕は暗黒の間に飛ばされてしまったみたい。\nこの暗黒の間は外からキーワードを入力しないと\n扉が開かない仕組みだから外に出ることが\nできなくなってしまったんだ。",
	"どうか暗黒の間まで来て、助けてくれないかな？",
	"地下1階から6階までの地図と暗黒の間のキーワードを\n示す暗号を送ったよ。ただ、各階の地図はあるけど、\nどの地図がなんの地図かは分からなかった。\n左上の印刷ボタンを押すと見れるはずだよ。",
	"それから、地図と今までに解いた暗号の文字コードは\n連動しているから、この暗号を解くための\n鍵になっているよ。",
	"今まで解いた暗号の文字コードはまとめておいたので、\nこれも一緒に送ったからね。",
	"最後に1つ注意点だけど、ダンジョン内には謎の霧が\n発生していて、視界が悪くなっているよ。",
	"正しい道順から外れると、落とし穴に落ちたり、\n罠に引っかかったりするかもしれないから、\n正しい道順を通るようにしてね。",
	"じゃあ、待ってるからよろし・・く・・・?%&$#*+#",
	"通信中にエラーが発生しました。"
]
let finFlag = false
...
const data_checkURL = URLList[12][1]
...
window.addEventListener('load',async ()=>{
	if(await checkURLFunction()){
		document.getElementById("answer").value = URLList[11][2]
		document.getElementById("answer").disabled = true
...
replay_button.addEventListener("click",async () =>{
	if(!replay_button_disabled){
		...
		typeWriter("comment-text",commentsList[commentFlag],100)
		document.body.style.background= "url('DOkfC0W8/background.jpg') no-repeat center center fixed"; // 黒色に透明度をつけて暗転
...
async function comment(){
	...
	if(commentFlag >= 0 && commentFlag <= commentsList.length -3){
		document.getElementById("comment-text").style.color = "#f5603b"
	}else{
		document.getElementById("comment-text").style.color = "white"
	}

	if (commentsList.length-1 > commentFlag && finFlag && Maincomment ){
		commentFlag ++
		

		if(commentFlag == 4 && isOn){
...
next_button.addEventListener("click", async () =>{
	if (!next_button_disabled){
		...
		if (next_status == 0){
			...
			if (response.ok) {
				URLList[11][2] = answer
				URLList[12][0] = "ランゲルを助けに!"
				URLList[12][1] = nextURL
				typeWriter("comment-text","解除成功",100);
				messageElement.style.opacity = 0;
				finFlag = false
				next_status = 1
