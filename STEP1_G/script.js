...
URLList[10][0] = document.title
URLList[10][1] = url[url.length - 1]
...
const commentsList = ["ログアウトキーワードを導き出そう。\nログアウトキーワードがわかったら右の解答欄に入力し、\n「解答する」ボタンを押してね!"]
let finFlag = false
let commentFlag = 0
...
const data_checkURL = URLList[11][1]
...
window.addEventListener('load',async ()=>{
	if(await checkURLFunction()){
		document.getElementById("answer").value = URLList[10][2]
		document.getElementById("answer").disabled = true
...
replay_button.addEventListener("click",async () =>{
	if(!replay_button_disabled){
		...
		typeWriter("comment-text",commentsList[commentFlag],100)
		document.body.style.background= "url('0PAlpjGH/background.jpg') no-repeat center center fixed"; // 黒色に透明度をつけて暗転
...
async function comment(){
	...
	if(next_status == 1 && finFlag && await checkAnswer() && isOn){
		document.getElementById("comment-frame").src = "0PAlpjGH/frame_system.png"
		typeWriter("comment-text","ランゲルは消えてしまった。",100);
		document.getElementById("rangelImg").style.opacity = 0
		Qcomment = true
		messageElement.style.opacity = 0
		next_status = 2
		finFlag = false
  ...
  if (isOn) {
	...
	if (!finFlag && !Qcomment){
		...
		if(next_status == 0){
			...
			if(!finFlag){
				typeWriter("comment-text","・・・",100);
			}else{
				messageElement.style.opacity = 1
				document.getElementById("comment-text").innerHTML = "・・・".split("\n").join('<br>')
			}
		}else if(next_status == 2 && await checkAnswer()){
			if(!finFlag){
				typeWriter("comment-text","次は「次へ」ボタンを押してGへ行こう。",100);
			}else{
				next_button.style.backgroundColor = "#05aa9084"
				next_button_disabled = false
				document.getElementById("comment-text").innerHTML = "次は「次へ」ボタンを押してGへ行こう。".split("\n").join('<br>')
			}
		}
...
async function checkAnswer(){
	...
	if (answer != ""){
		const response = await fetch('/answer/STEP1_G', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
	...
	if (!next_button_disabled){
		...
		if (next_status == 0){
			...
			if (answer != ""){
				replay_button_disabled = true
				replay_button.style.backgroundColor = "#cccccc84"
				response = await fetch('/answer/STEP1_G', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
			...
			if (response.ok) {
				URLList[10][2] = answer
				URLList[11][0] = "ランゲルを助けに!"
				URLList[11][1] = nextURL
				typeWriter("comment-text","・・・",300);
				messageElement.style.opacity = 0;
				finFlag = false
				next_status = 1
				next_button.innerText = "次へ⇨"
