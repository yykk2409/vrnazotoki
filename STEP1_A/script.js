...
URLList[4][0] = document.title
URLList[4][1] = url[url.length - 1]
...
const data_checkURL = URLList[5][1]
...
window.addEventListener('load',async ()=>{
	if(await checkURLFunction()){
		document.getElementById("answer").value = URLList[4][2]
		document.getElementById("answer").disabled = true
		next_button.style.backgroundColor = "#05aa9084"
		next_button_disabled = false
		next_button.innerText = "Bへ行く⇨"
		next_status = 2
...
async function comment(){
	...
	if(next_status == 1 && finFlag && await checkAnswer() && isOn){
		typeWriter("comment-text","次は「Bへ行く」ボタンを押してBへ行こう。",100);
		Qcomment = true
		messageElement.style.opacity = 0
		next_status = 2
		finFlag = false
...
async function toggleSwitch() {
  ...
  if (isOn) {
	...
	document.body.style.background= "url('uBMrjeP8/background.jpg') no-repeat center center fixed"; // 黒色に透明度をつけて暗転
	...
	if (!finFlag && !Qcomment){
		...
		if(next_status == 0){
			...
			if(!finFlag){
				...
				next_button.style.backgroundColor = "#05aa9084"
				next_button_disabled = false
				document.getElementById("comment-text").innerHTML = "次は「Bへ行く」ボタンを押してBへ行こう。".split("\n").join('<br>')
			}
		}
...
async function checkAnswer(){
	...
	if (answer != ""){
		const response = await fetch('/answer/STEP1_A', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
	...
	if (!next_button_disabled){
		...
		if (next_status == 0){
			...
			if (answer != ""){
				replay_button_disabled = true
				replay_button.style.backgroundColor = "#cccccc84"
				response = await fetch('/answer/STEP1_A', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
