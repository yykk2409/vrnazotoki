...
URLList[6][0] = document.title
URLList[6][1] = url[url.length - 1]
...
const data_checkURL = URLList[7][1]
...
window.addEventListener('load',async ()=>{
	if(await checkURLFunction()){
		document.getElementById("answer").value = URLList[6][2]
		document.getElementById("answer").disabled = true
		next_button.style.backgroundColor = "#05aa9084"
		next_button_disabled = false
		next_button.innerText = "Dへ行く⇨"
		next_status = 2
...
replay_button.addEventListener("click",async () =>{
	if(!replay_button_disabled){
		...
		typeWriter("comment-text",commentsList[commentFlag],100)
		document.body.style.background= "url('020Gv8gM/background.jpg') no-repeat center center fixed"; // 黒色に透明度をつけて暗転
...
async function comment(){
	...
	if(next_status == 1 && finFlag && await checkAnswer() && isOn){
		typeWriter("comment-text","次は「Dへ行く」ボタンを押してDへ行こう。",100);
		Qcomment = true
		messageElement.style.opacity = 0
		next_status = 2
		finFlag = false
	...
	if (!finFlag && !Qcomment){
		...
		if(next_status == 0){
			...
			if(!finFlag){
				...
				next_button.style.backgroundColor = "#05aa9084"
				next_button_disabled = false
				document.getElementById("comment-text").innerHTML = "次は「Dへ行く」ボタンを押してDへ行こう。".split("\n").join('<br>')
			}
		}
...
async function checkAnswer(){
	...
	if (answer != ""){
		const response = await fetch('/answer/STEP1_C', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
	...
	if (!next_button_disabled){
		...
		if (next_status == 0){
			...
			if (answer != ""){
				replay_button_disabled = true
				replay_button.style.backgroundColor = "#cccccc84"
				response = await fetch('/answer/STEP1_C', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
					},
